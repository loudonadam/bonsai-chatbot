...
3) **Copy the server binary where quick_launch can see it**
   ...
   - Seeing `ggml_vulkan: Found 2 Vulkan devices` is normal on systems with an iGPU + dGPU. The quick launch script now auto-picks the RX 7900 XTX (or any GPU whose name matches `-PreferredVulkanGpuPattern`, default `"7900"`) when multiple devices are reported. If you want to override the match or be explicit, use `-VulkanDevice <index>` or set `-PreferredVulkanGpuPattern "RX 7900"` to line up with your llama.cpp device list. You can also restrict ICDs via `-VkIcdFilenames "<path to AMD ICD json>"` so only the discrete GPU is visible.
