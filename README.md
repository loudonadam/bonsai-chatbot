...
3) **Copy the server binary where quick_launch can see it**
   ...
   - Seeing `ggml_vulkan: Found 2 Vulkan devices` is normal on systems with an iGPU + dGPU. The quick launch script now auto-picks the RX 7900 XTX (or any GPU whose name matches `-PreferredVulkanGpuPattern`, default `"7900"`) and re-runs the self-test once. If it still fails, run with an explicit index: `.\scripts\quick_launch.ps1 -VulkanDevice 1` (the index printed by llama.cpp is 0-based), or restrict ICDs via `-VkIcdFilenames "<path to AMD ICD json>"` so only the discrete GPU is visible.
