...
3) **Copy the server binary where quick_launch can see it**
   - Option A: copy **everything** from `build\bin\Release` into this repo’s `scripts\` folder: `llama-server.exe` **and all DLLs** (`ggml*.dll`, `ggml-vulkan*.dll`, `llama.dll`, `mtmd.dll`, etc.). Missing or blocked DLLs cause self-test errors like exit code `-1073741515 (0xC0000135)`, even if the log shows Vulkan devices detected.
   ...
   - Seeing `ggml_vulkan: Found 2 Vulkan devices` is normal on systems with an iGPU + dGPU. If the self-test still fails, it’s almost always missing/blocked DLLs. To force the discrete GPU when two devices are present, set an env var before launching (supported on recent llama.cpp builds): `set GGML_VULKAN_DEVICE=1` (0-based index), or restrict ICDs via `VK_ICD_FILENAMES` (semicolon-separated ICD JSON paths; use your vendor’s dGPU ICD). In the helpers, use `scripts\quick_launch.ps1 -VulkanDevice 1` or `-VkIcdFilenames "<path-to-icd.json>"` (add `-ShowVulkanEnv` to confirm).
