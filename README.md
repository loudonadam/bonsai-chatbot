...
3) **Copy the server binary where quick_launch can see it**
   ...
   - Seeing `ggml_vulkan: Found 2 Vulkan devices` is normal on systems with an iGPU + dGPU. The quick launch script now auto-picks the RX 7900 XTX (or any GPU whose name matches `-PreferredVulkanGpuPattern`, default `"7900"`) and re-runs the self-test once. If no pattern matches, it falls back to the highest device index (typically the discrete GPU) even when llama.cpp only prints the count. If it still fails, be explicit: `.\scripts\quick_launch.ps1 -VulkanDevice 1` (0-based from the llama.cpp list), or restrict ICDs via `-VkIcdFilenames "<path to AMD ICD json>"` (examples: `C:\Windows\System32\amdvlk64.dll`, `C:\Windows\System32\amd_icd64.json`, or `C:\Program Files\AMD\...\amd_icd64.json`) so only the discrete GPU is visible. To see the exact device list, run `.\scripts\llama-server.exe --version` from the repo root. If the self-test keeps failing, grab `logs\quick-launch-vulkan.log` (auto-written during quick launch) to see which DLLs were found, the parsed device list/count, and which GPU index was chosen; include that log when asking for help.
   ...
   - Need to rebuild llama.cpp? Re-run the CMake configure step with the Vulkan flag so AMD stays available: `cmake -B build -G "Visual Studio 17 2022" -A x64 -DLLAMA_CURL=OFF -DGGML_VULKAN=ON` (or add `-DLLAMA_VULKAN=ON` if your tree still uses the old name), then `cmake --build build --config Release -j`. Copy everything from `build\bin\Release` next to `llama-server.exe` after rebuilding.
